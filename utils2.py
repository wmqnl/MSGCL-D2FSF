...
#                 labels.append(np.array([cache[-1]], dtype=str) )
            ...
            if not features_block:
                ...
                features,fea_indices,fea_values= dense_tensor_to_sparse(features)#fea_values不作为load_data 返回值

            features_list.append(features)
            features_indices_list.append(fea_indices)
    ...
    for i1 in range(len(type_list)):
        for i2 in range(len(type_list)):
            adj_list[i1][i2],indices,values = sparse_mx_to_torch_sparse_tensor(
                adj_all[sum(len_list[:i1]): sum(len_list[:i1 + 1]),
                        sum(len_list[:i2]): sum(len_list[:i2 + 1])]
            )
            indices_list.append(indices)
            adj_values_list.append(values)
...
#     return adj_list, features_list, Labels, idx_train, idx_val, idx_test, idx_map_list[0]
    return adj_list, features_list,indices_list,adj_values_list,features_indices_list
...
def sparse_mx_to_torch_sparse_tensor(sparse_mx):
    ...
    return torch.sparse.FloatTensor(indices, values, shape),indices,values
